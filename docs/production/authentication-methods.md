...
1. Tell Aloha how to connect to your SAML provider(s) by filling
   out the section of `/etc/zulip/settings.py` on your Aloha server
...
1. Install the certificate(s) required for SAML authentication. You
   ...
   1. On your Aloha server, `mkdir -p /etc/zulip/saml/idps/`
   2. Put the IDP public certificate in `/etc/zulip/saml/idps/{idp_name}.crt`
   3. (Optional) Put the Aloha server public certificate in `/etc/zulip/saml/zulip-cert.crt`
...
`{"subdomain": "yourzuliporganization"}` to let Aloha know which
