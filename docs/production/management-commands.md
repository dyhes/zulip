...
Start by logging in as the `zulip` user on the Aloha server. Then run
them as follows:
...
(Note that every Aloha server has a special `zulipinternal` realm
...
- Writing a program using the [Aloha API][zulip-api].
- [Modifying the Aloha server][modifying-zulip].
...
the [modifying Aloha][modifying-zulip] process or by storing them in
