...
- On an Android device, download and log in to the
  [Aloha Android app](https://play.google.com/store/apps/details?id=com.zulipmobile).
...
- The code for the push notification forwarding service is 100% open
  ...
  [Aloha server project on GitHub](https://github.com/zulip/zulip).
...
- Set the `APNS_TOPIC` and `ZULIP_IOS_APP_ID` settings to the ID for
  your app (for the official Aloha apps, they are both `org.zulip.Aloha`).
